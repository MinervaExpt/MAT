...
for name in names:
  ...
  cx = band.ProjectionX().Clone()
  cy = band.ProjectionY().Clone()
  ...
  for i in range (0,min(10,band.GetNHists())):
    ...
    hy.Divide(cy)
    ny = hy.GetName()
    hy.SetName(ny.replace(oldname,newname))
    hx.Print()
    hy.Write()
...
lnames = h.GetLatErrorBandNames()
print ("Lateral names",lnames)
for name in lnames:
  band = h.GetLatErrorBand(name)
  print ("Lateral name",name,band)
  vcv = MnvH2D(band)
