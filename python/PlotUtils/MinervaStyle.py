"""
  MinervaStyle.py:
   Loads the MINERvA plot style from PlotUtils.
...
def SetStyle(path_to_plotutils):
	...
	ROOT.gROOT.Macro("PlotStyle.C")


if "PLOTUTILSROOT" in os.environ:
	SetStyle("%s/PlotUtils" % os.path.expandvars("$PLOTUTILSROOT"))
else:
	print >> sys.stderr, "WARNING: $PLOTUTILSROOT is not defined in the current environment.  MinervaStyle cannot be loaded automatically."
