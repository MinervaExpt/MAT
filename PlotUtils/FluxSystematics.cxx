...
namespace MAT{
  ...
  std::vector<T*> GetFluxSystematics(typename T::config_t chain, 
    ...
    for (unsigned int i = 0; i < n_universes; ++i)
      ret.push_back(new MAT::FluxUniverse<T>(chain, nsigma, i));
    return ret;
  }
  ...
  std::map< std::string, std::vector<T*> > GetFluxSystematicsMap(typename T::config_t chain, 
    ...
    for (unsigned int i = 0; i < n_universes; ++i)
      ret["Flux"].push_back(new MAT::FluxUniverse<T>(chain, nsigma, i));
    return ret;
  }
...
// Constuctor
template <class T>
MAT::FluxUniverse<T>::FluxUniverse(typename T::config_t chw,
...
template <class T>
double MAT::FluxUniverse<T>::GetFluxAndCVWeight(double Enu,
                                                      int nu_pdg) const {
  ...
  double sysWgt = MAT::flux_reweighter(T::GetPlaylist(),nu_pdg,T::UseNuEConstraint(),T::GetNFluxUniverses()).GetFluxErrorWeight(Enu, nu_pdg, 
                                                                                   m_universe_number);
  return sysWgt*T::GetFluxAndCVWeight();
}

template <class T>
double MAT::FluxUniverse<T>::GetWeightRatioToCV() const
{
  ...
  // FRW::GetFluxErrorWeight() returns a weight which is the ratio for a given Enu b/w the sys. universe wgt and the CV wgt
  // So, the correct usage is to multiply this weight by the CV weight, not to replace the CV weight with this one
  return MAT::flux_reweighter(T::GetPlaylist(),nu_pdg,T::UseNuEConstraint(),T::GetNFluxUniverses()).GetFluxErrorWeight(Enu, nu_pdg,
                                                                                   m_universe_number);
}


template <class T>
void MAT::FluxUniverse<T>::SetUniverseNumber(int i) {
  m_universe_number = i;
}


template <class T>
std::string MAT::FluxUniverse<T>::ShortName() const {
  return "Flux";
}


template <class T>
std::string MAT::FluxUniverse<T>::LatexName() const {
  return "Flux";
}
